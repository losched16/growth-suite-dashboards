...
## Addendum 3 — JotForm applications imported (2026-08-21)
Four JotForm "Application for Admission" PDFs (Rocco Zancolli, William Corwell, Kahlani Palmer, Rayne Coil). The GHL "Application" is a **survey** (`DeXretNk9kKihdxeUrub`) with a required live $100 payment step and two published workflows (*Application Submitted – Confirmation* emails the parent; *Application – Pipeline Opportunity* creates a card) — replaying it would email late confirmations and card enrolled kids, and would most likely be rejected without a payment. So they were imported the supported way (`scripts/mcs-import-jotform-applications.mjs`): every answer into the contact fields the survey maps to, the PDF in the media library + a contact note with the JotForm payment ref, blanks filled (DOB, Parent 2, address), and a Lower-El "Application Submitted" card for Rocco (the only non-enrolled applicant). They will NOT appear in the survey Submissions view — that view only holds real survey submissions.
Also fixed: the `grade_level_of_interest` field (and therefore the survey dropdown) had only "Adolescent (12-14 yrs)" left as an option — restored the 5 programs. Flag: Rayne Coil's application lists qljones.1992@outlook.com; the contact has qljones@outlook.com.
