// Discount-evaluation engine — DASHBOARDS COPY.
//
// Mirror of growth-suite-parent-portal/lib/billing/discounts.ts. The two
// apps don't share imports (separate Next.js packages), so we duplicate
// the helper the same way fee-math.ts is duplicated. If you change one,
// update the other.
//
...
//   - code       : only applied if the operator entered the matching code
...
export interface DiscountInput {
  ...
  formSlug?: string;
  // Code the operator typed in on the invoice form (case-insensitive).
...
//
// Unknown keys cause the policy to fail-closed (skip).
