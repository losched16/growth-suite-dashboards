...
function AiAnalysisPanel({
  ...
  // Guard so the auto-trigger fires exactly once per mounted panel.
  // Without this, React StrictMode + state changes would double-fire.
  ...
  // Auto-fire on mount when no cached analysis exists. This is the
  // school's primary experience — the committee should never have to
  // click a button to see the analysis. If submission's analysis was
  // already generated by the parent-portal submit hook, this no-ops.
  useEffect(() => {
    if (autoFiredRef.current) return;
    if (analysis || busy || err) return;
