// /school/[locationId]/documents — office manager for "important
// documents" pushed to the parent portal. Upload once, target with the
// notifications-style audience (program / classroom / grade / tag /
// family, AND-OR) plus optional excludes; parents see matching documents
// in their portal under Documents → "From your school". Targeting is
// evaluated live per family, so tag/program changes flow automatically.
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5">
        <div>
          <Link href={`/school/${locationId}/dashboard?chrome=none`}
          ...
          <p className="mt-1 text-xs text-slate-600">
            Files you share here appear in each matching family&rsquo;s parent portal under
        ...
        <ShareDocument schoolId={school.id} options={options} />

        <section className="space-y-2">
          <h2 className="text-sm font-semibold text-slate-700">Shared documents ({docs.length})</h2>
          {docs.length === 0 ? (
                ...
                <li key={d.id} className={`px-4 py-3 flex items-center justify-between gap-3 flex-wrap ${d.is_active ? '' : 'opacity-60'}`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <a href={`/api/admin/schools/${school.id}/shared-documents/${d.id}`}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DocActions schoolId={school.id} docId={d.id} isActive={d.is_active} />
                  </div>
                </li>
              ))}
            </ul>
