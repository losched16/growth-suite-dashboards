...
// School-iframe-context version of NewFormForm from the admin route.
// Identical UX; differs only in: (a) posts to /api/school/forms/create
// (school session) instead of the admin endpoint, and (b) on success
// redirects to /school/{locationId}/forms/{newId} so the parent iframe
// stays inside the school namespace.
...
export function NewFormForm({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const r = await fetch(`/api/school/forms/create`, {
          method: 'POST',
        ...
        if (!body.id) throw new Error('No form id returned from server.');
        // Land on the editor for the new form (school namespace)
        router.push(`/school/${locationId}/forms/${body.id}?chrome=none`);
      } catch (e) {
